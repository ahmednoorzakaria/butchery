...
// Payment types
const PAYMENT_TYPES = ['CASH', 'MPESA'] as const;
...
async function main() {
  ...
  for (const itemData of INVENTORY_ITEMS) {
    const item = await prisma.inventoryItem.create({
      data: {
        ...
        lowStockLimit: faker.number.int({ min: 10, max: 30 }),
      },
    });
  ...
  for (const name of CUSTOMER_NAMES) {
    const customer = await prisma.customer.create({
      data: {
        name,
        phone: `+254${faker.number.int({ min: 700000000, max: 799999999 })}`,
      },
    });
  ...
  for (let i = 0; i < 3000; i++) {
    ...
    const randomDate = faker.date.between({ from: threeMonthsAgo, to: today });
    
    // Randomly select customer and user
    const customer = faker.helpers.arrayElement(customers);
    const user = faker.helpers.arrayElement([ahmed, jon]);
    
    // Generate realistic sale amounts based on butchery business
    const baseAmount = faker.number.float({ min: 200, max: 5000, fractionDigits: 2 });
    const discount = faker.number.float({ min: 0, max: baseAmount * 0.15, fractionDigits: 2 });
    ...
    const selectedItems = faker.helpers.arrayElements(inventoryItems, numItems);
    
    for (const item of selectedItems) {
      const quantity = faker.number.float({ min: 0.5, max: 5, fractionDigits: 1 });
  ...
  for (const item of inventoryItems) {
    ...
    const stockInCount = faker.number.int({ min: 3, max: 8 });
    for (let i = 0; i < stockInCount; i++) {
      const transactionDate = faker.date.between({ from: threeMonthsAgo, to: today });
    ...
    const stockOutCount = faker.number.int({ min: 2, max: 6 });
  ...
  for (let i = 0; i < 500; i++) {
    ...
    await prisma.customerTransaction.create({
      data: {
        ...
        reason: faker.helpers.arrayElement(TRANSACTION_REASONS),
        createdAt: faker.date.between({ from: threeMonthsAgo, to: today }),
      },
    });
  }
  ...
  for (let i = 0; i < 100; i++) {
    const expenseDate = faker.date.between({ from: threeMonthsAgo, to: today });
    await prisma.expense.create({
      data: {
        amount: faker.number.float({ min: 500, max: 50000, fractionDigits: 2 }),
        reason: faker.helpers.arrayElement(expenseReasons),
  ...
  console.log(`   - Sales span: ${format(threeMonthsAgo, 'MMM dd, yyyy')} to ${format(today, 'MMM dd, yyyy')}`);
}
