...
router.get("/profit-loss", authenticateToken, async (req, res) => {
  try {
    const { start, end } = req.query;
    let startDate = start ? new Date(start as string) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    ...
    sales.forEach(sale => {
      sale.items.forEach(item => {
        ...
        const cost = item.quantity * ((item.item as any).basePrice || item.price * 0.7); // Assume 70% of selling price if no base price
...
router.get("/inventory-valuation", authenticateToken, async (req, res) => {
  try {
    ...
    const categoryBreakdown: Record<string, { name: string; count: number; value: number }> = {};
...
router.get("/enhanced-inventory", authenticateToken, async (req, res) => {
  try {
    const inventory = await prisma.inventoryItem.findMany();
...
router.get("/customer-analysis", authenticateToken, async (req, res) => {
  try {
    const { start, end } = req.query;
    let startDate = start ? new Date(start as string) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    let endDate = end ? new Date(end as string) : new Date();
...
router.get("/loss-analysis", authenticateToken, async (req, res) => {
  try {
    const inventory = await prisma.inventoryItem.findMany();
    ...
    const highRiskItems = inventory.filter(item => {
      return item.quantity <= (item.lowStockLimit || 5) || 
             item.quantity === 0 ||
             ((item as any).expiryDate && new Date((item as any).expiryDate) < new Date());
    });

    const wasteRiskItems = inventory.filter(item => {
      return (item as any).expiryDate && new Date((item as any).expiryDate) < new Date();
    });

    const expiryRiskItems = inventory.filter(item => {
      if (!(item as any).expiryDate) return false;
      const daysUntilExpiry = Math.ceil((new Date((item as any).expiryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
...
router.get("/inventory-projections", authenticateToken, async (req, res) => {
  try {
    const inventory = await prisma.inventoryItem.findMany();

    let totalProjectedRevenue = 0;
    let totalProjectedCost = 0;
    const itemProjections: Array<{
      name: string;
      category: string;
      totalQuantity: number;
      totalRevenue: number;
