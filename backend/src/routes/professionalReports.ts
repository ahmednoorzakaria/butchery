...
const router = Router();
const reportService = new ProfessionalReportService();

// GET /professional-reports/excel/:date - Generate and download Excel report
router.get("/excel/:date", authenticateToken, async (req, res) => {
  try {
...
router.get("/pdf/:date", authenticateToken, async (req, res) => {
  try {
    const { date } = req.params;
    const reportDate = date ? new Date(date) : new Date();
